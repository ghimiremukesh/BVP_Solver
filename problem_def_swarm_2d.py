...
class config_NN(config_prototype):
    def __init__(self, N_states, time_dependent):
        ...
        self.Ns = {'train': 1800, 'val': 600, 'test': 1}
...
class setup_problem(problem_prototype):
    ...
    def U_star(self, X_aug):
        ...
        dHdu = np.array([[Hu_1[1, :] - Hu_1[0, :]], [Hu_1[0, :] - Hu_1[1, :]]]).reshape(2, -1)
        dHdd = np.array([[Hu_2[1, :] - Hu_2[0, :]], [Hu_2[0, :] - Hu_2[1, :]]]).reshape(2, -1)
    ...
    def aug_dynamics(self, t, X_aug):
        ...
        dX1_dt = np.vstack((U1[1] * x1_2 - U1[0] * x1_1,
                            U1[0] * x1_1 - U1[1] * x1_2))
        dX2_dt = np.vstack((U2[1] * x2_2 - U2[0] * x2_1,
                           U2[0] * x2_1 - U2[1] * x2_2))
        ...
        dA1_dt = np.array([U1[0] * (A_1[0] - A_1[1]),
                           U1[1] * (A_1[1] - A_1[0])])
        dA2_dt = np.array([U2[0] * (A_2[0] - A_2[1]),
                           U2[1] * (A_2[1] - A_2[0])])

        dVdt = np.ones((len(t)))   # add instantaneous loss
