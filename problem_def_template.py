...
class problem_prototype:
    ...
    def sample_X0(self, Ns):
        ...
        # D = bounds.shape[0]
        sampler = LatinHypercube(d=self.X0_lb.shape[0])
    ...
    def sample_test(self):
        num_states = self.X0_lb.shape[0]
        x0_1 = (1/num_states) * np.ones((1, self.X0_lb.shape[0]))
        X0 = np.concatenate((x0_1, x0_1), axis=1)  # uniform over all region

        return X0.T


    def sample_uniform(self, Ns):
        assert Ns == 1, "this is just for 1 trajectory"
    ...
    def Hamiltonian(self, t, X_aug):
        ...
        H =  np.sum(X_aug[self.N_states:2*self.N_states] * F[:self.N_states],
