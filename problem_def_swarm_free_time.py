...
class setup_problem(problem_prototype):
    def __init__(self):
        ...
        self.X0_lb = 0.25 * np.ones((self.N_states,))
        self.X0_ub = 0.25 * np.ones((self.N_states,))
        ...
        self.beta = 0.2
        self.check_final_state = lambda X: 100 if np.max(X) > self.beta else -100  # should be zero.
    ...
    def make_bc(self, X0_in):
        def bc(X_aug_0, X_aug_T):
            ...
            gT = np.array(self.check_final_state(del_XT)).reshape(1, )
            ...
            #                         np.array([self.alpha]),
            #                         np.array([-2 * (XT[3] - 18)])))
            return np.concatenate((X0 - X0_in, AT, VT, gT))
    ...
    def aug_dynamics(self, t, X_aug):
        ...
        dVdt = -np.ones((len(t)))  # dvdt = -1
        Tf = tf * np.ones((len(t)))

        return np.vstack((tf * dX1_dt, tf * dX2_dt, tf * dA1_dt, tf * dA2_dt, tf * dVdt, Tf))
