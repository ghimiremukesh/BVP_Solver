...
while N_sol < Ns:
    ...
    try:
        ...
        if system == "swarm":
            t_guess = np.array([0.])
            X_guess = np.vstack((X0.reshape(-1, 1), np.zeros((8, 1)), np.ones((1, 1))))
            # X_guess = np.vstack((X0.reshape(-1,1),
            #                      np.zeros((N_states+1, 1))))
        elif system == "swarm-2d":
            t_guess = np.array([0.])
            X_guess = np.vstack((X0.reshape(-1, 1), np.ones((4, 1)), np.ones((1, 1))))
        ...
        A_OUT = np.hstack((A_OUT, SOL.y[2 * N_states:4 * N_states]))
        ...
        X0_pool[:, N_sol] = problem.sample_X0(1).flatten()
...
if save_data:
    save_path = 'examples/' + system + '/data_' + data_type + str(problem.t1) + 's_new' + '.mat'
